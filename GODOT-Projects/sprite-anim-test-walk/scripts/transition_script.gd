...
@onready var camera = $"SubViewportContainer/SubViewport/transition-camera"
...
func _ready() -> void:
	...
	lines = [
	StaticData.textData["transition_start_dialogue"]["start_dialogue_1"],
	]
...
func _process(_delta: float) -> void:
	...
	if Input.is_action_just_pressed("KeyF"):
		if GlobalVariables.insideGate == true:
			...
			await get_tree().create_timer(2.0).timeout
			get_tree().change_scene_to_file("res://scenes/end_transition_animation.tscn");
		else:
			pass
	else:
		pass
...
func _on_dialogue_activator_1_body_entered(_body: Node3D) -> void:
	lines = [
		StaticData.textData["transition_dialogue_1"]["cube_dialogue_1"],
		StaticData.textData["transition_dialogue_1"]["cube_dialogue_2"],
		StaticData.textData["transition_dialogue_1"]["cube_dialogue_3"],
		StaticData.textData["transition_dialogue_1"]["cube_dialogue_4"],
		StaticData.textData["transition_dialogue_1"]["cube_dialogue_5"],
	]
	textBox.start_dialogue(lines)
...
func _on_gate_area_2_body_entered(_body: Node3D) -> void:
	$SubViewportContainer/SubViewport/GateToRealm2.material_overlay = outline_material
	GlobalVariables.insideGate = true
	GlobalVariables.pressFdisplay = true

func _on_gate_area_2_body_exited(_body: Node3D) -> void:
	$SubViewportContainer/SubViewport/GateToRealm2.material_overlay = null
	GlobalVariables.insideGate = false
	GlobalVariables.pressFdisplay = false
